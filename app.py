...
import numpy as np  # Keep numpy for handling descriptor arrays
...
if not os.path.exists(
        'face_data/attendance_logs'):  # Only need logs, registered_face not needed for server-side storing
...
def insert_attendance_record(student_id, date, time, image_path, latitude, longitude):
    conn = get_db_connection()
    conn.execute(
        "INSERT INTO attendance (student_id, date, time, image_path, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?)",
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user_data and user_data['role'] == 'student':
            student_data = conn.execute('SELECT date_of_birth FROM students WHERE user_id = ?',
                                        (user_data['id'],)).fetchone()
...
def register():
    if request.method == 'POST':
        ...
        if conn.execute('SELECT COUNT(*) FROM students WHERE roll_number = ?', (roll_number,)).fetchone()[0] > 0:
            flash(f"Roll Number '{roll_number}' is already registered. Please login or use a unique Roll Number.",
                  "error")
            conn.close()
            return redirect(url_for('register'))
        ...
        try:
            ...
            conn.execute(
                "INSERT INTO students (user_id, name, roll_number, section, date_of_birth, face_descriptor) VALUES (?, ?, ?, ?, ?, ?)",
                (student_user_id, name, roll_number, section, date_of_birth, str(face_descriptor)))  # Store as string
            conn.commit()
            conn.close()

            # For registration, we might not need to save the raw image file on the server anymore
            # as the descriptor is the primary unique identifier.
            # However, if you want to keep the image for admin review, you can save it.
            # If keeping, ensure the 'face_data/registered_face' directory is created.
            # For now, client-side recognition means the server doesn't need to process the image directly.

            flash(f"✅ Registration successful! Your username is '{roll_number}' and password is your Date of Birth.",
                  "success")
            return redirect(url_for('login'))
...
def mark_attendance():
    ...
    student_data = conn.execute(
        'SELECT id, name, roll_number, section, face_descriptor FROM students WHERE user_id = ?',
    ...
    if request.method == 'POST':
        ...
        if attendance_today > 0:
            flash(f"⚠️ Attendance already marked for {student_name} ({student_roll_number}) today ({today_date}).",
                  "warning")
            return redirect(url_for('my_attendance'))
        ...
        try:
            ...
            if is_match:
                ...
                insert_attendance_record(student_id, today_date, current_time,
                                         os.path.join(attendance_log_dir, filename), latitude, longitude)
                flash(
                    f"✅ Attendance marked for {student_name} ({student_roll_number}) for {today_date} at {current_time}!",
                    "success")
                return redirect(url_for('my_attendance'))
            else:
...
def admin_dashboard():
    ...
    students = conn.execute(
        'SELECT id, name, roll_number, section, date_of_birth FROM students ORDER BY name').fetchall()
    ...
    return render_template('admin_dashboard.html',
                           students_and_records={'students': students, 'attendance_records': attendance_records})
...
def delete_student(student_id):
    ...
    try:
        ...
        # NOTE: If you decide to save registered face images on the server for admin review,
        # you would need to fetch the image path here (e.g., from a new column in students table)
        # and delete it from the file system.
